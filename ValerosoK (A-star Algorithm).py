...
def pop_up_window(app):
    def start_button_action():
        ...
        win.destroy()
    win = Toplevel()
    win.wm_title("Welcome. Please follow the guide.")


    Label(win, text="Step 1: Click a starting point.",
          font=("Times New Roman", 13), pady=5, padx=10).pack()
    Label(win, text="Step 2: Click an end point.", font=(
        "Times New Roman", 13), pady=5, padx=10).pack()
    Label(win, text="Step 3: Select Obstacles", font=(
        "Times New Roman", 13), pady=5, padx=10).pack()
    Label(win, text="Click and drag. Then click again to Stop.", padx=25).pack()
...
class App:
    ...
    def __init__(self, master):
        self.master = master
        master.wm_title("A-star Algorithm")
        self.buttons = []
        self.start = []
        self.goal = []
        self.obstacles = []
    ...
    def button_operation(self, row, column):
        ...
        if self.mode == 0:
            ...
            self.mode = 2
            self.buttons[row][column].configure(bg='grey')
    ...
    def add_obstacle(self, row, column, event):
        ...
        if self.mode == 3:
            ...
            self.buttons[row][column].configure(bg='gold')
    ...
    def find_neighbors(self, current, obstacles):
        ...
        if 0 <= down_right_neighbor[0] < 25 and 0 <= down_right_neighbor[
            1] < 25 and down_right_neighbor not in self.obstacles:
        ...
        if 0 <= up_right_neighbor[0] < 25 and 0 <= up_right_neighbor[
            1] < 25 and up_right_neighbor not in self.obstacles:
        ...
        if 0 <= down_left_neighbor[0] < 25 and 0 <= down_left_neighbor[
            1] < 25 and down_left_neighbor not in self.obstacles:
    ...
    def reconstruct_path(self, cameFrom, current):
        ...
        while current != self.start:
            self.buttons[current[0]][current[1]].configure(bg='brown')
            ...
            for node in neighbors:
                ...
                if tentative_gScore < g_score[node_row][node_column]:
                    ...
                    if node not in open_set:
                        self.buttons[node[0]][node[1]].configure(bg='silver')
